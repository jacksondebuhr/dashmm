...
class DualTree {
 public:
  ...
  static hpx_addr_t partition(RankWise<dualtree_t> global_tree) {
		...
		std::cout<<"rank "<<hpx_get_my_rank()<<" in tree partition"<<std::endl;
		hpx_lco_wait(done);
    hpx_lco_delete_sync(done);	*/	
    ...
    hpx_bcast_lsync(create_dual_tree_, retval, &tree_gas);
		...
		std::cout<<"rank "<<hpx_get_my_rank()<<" end tree partition"<<std::endl;
 ...
 private:
  ...
  void distribute_points(int num_ranks, const int *global, int len) {
    ...
    const int *t = &global[len]; // Target counts
		...
		// create and partition a Cell Grid from the cumulative counts
		Distributer distributer;
		distributer.setup(num_ranks, counts, len);
		int* rm = distributer.partition();
...
/*
  ...
  static int create_dual_tree_handler(hpx_addr_t rwtree) {
		std::cout<<"rank "<<hpx_get_my_rank()<<" in create_dual_tree"<<std::endl;
...

    {
      ...
      hpx_lco_get(tree->unif_count_, sizeof(int) * (tree->dim3_ * 2),
                  tree->unif_count_src());
      tree->distribute_points(num_ranks, tree->unif_count_src(), tree->dim3_);////////////////////////////////////////////////////////debug

			std::cout<<"rank "<<hpx_get_my_rank()<<" distributed points"<<std::endl;
      ...
      if (!tree->same_sandt_) {
        ...
        targettree_t::init_point_exchange_same_s_and_t(tree->target_tree_,
			...
			std::cout<<"rank "<<hpx_get_my_rank()<<" exchanged points"<<std::endl;
      ...
      assert(dual_tree_complete != HPX_NULL);
			...
			std::cout<<"rank "<<hpx_get_my_rank()<<" sent points"<<std::endl;
        ...
        if (tree->rank_of_unif_grid(i) == rank) {
          ...
          if (*(tree->unif_count_tar(i)) == 0) {
            ...
            hpx_call_when_with_continuation(nt[i].complete(),
				...
				std::cout<<"rank "<<hpx_get_my_rank()<<" : i="<<i<<std::endl;
      ...
      hpx_lco_delete_sync(dual_tree_complete);
			...
			std::cout<<"rank "<<hpx_get_my_rank()<<" sent points in parcell"<<std::endl;
