...
The \texttt{M->L} operation is used to translate summaries of a set of sources
...
The \texttt{L->L} operation is used to translate a local expansion that applies
for a parent into a local expansion that applies in the child's volume.

The \texttt{M->T} operation computes the effect of a multipole expansion on
a set of targets.

The \texttt{L->T} operation computes the effect of a local expansion on a
set of targets.

The \texttt{S->T} operation computes the direct effect of a set of sources on
a set of targets. If a method only schedules \texttt{S->T} operations, it will
be the same as the direct summation method.

The \texttt{M->I} operation transforms the summary information on the source
...
The \texttt{I->I} operation translates the intermediate representation on the
...
The \texttt{I->L} operation translates the intermediate representSation on the
...
node of this object with \texttt{Operation::StoM}, and assign the resulting
...
\noindent Connect the particles DAG node of \texttt{source} to the normal DAG
node of this object with \texttt{Operation::StoL}, and assign the resulting
edge the given \texttt{weight}. It is an error to call this method if this
object does not own a normal DAG node, and if the \texttt{source} does not
own a particles DAG node.
...
node of this object with \texttt{Operation::MtoM}, and assign the resulting
edge the given \texttt{weight}. It is an error to call this method if this
object does not own a normal DAG node, and if the \texttt{source} does not
...
\noindent Connect the normal DAG node of \texttt{source} to the normal DAG
node of this object with \texttt{Operation::MtoL}, and assign the resulting
edge the given \texttt{weight}. It is an error to call this method if this
object does not own a normal DAG node, and if the \texttt{source} does not
own a normal DAG node.
...
\noindent Connect the normal DAG node of \texttt{source} to the normal DAG
node of this object with \texttt{Operation::LtoL}, and assign the resulting
edge the given \texttt{weight}. It is an error to call this method if this
object does not own a normal DAG node, and if the \texttt{source} does not
own a normal DAG node.
...
node of \texttt{target} with \texttt{Operation::MtoT}, and assign the
...
\noindent Connect the normal DAG node of this object to the particles DAG
node of \texttt{target} with \texttt{Operation::LtoT}, and assign the
resulting edge the given \texttt{weight}. It is an error to call this method
if this object does not own a normal DAG node, and if the \texttt{target}
does not own a particles DAG node.
...
DAG node of this object with \texttt{Operation::StoT}, and assign the
resulting edge the given \texttt{weight}. It is an error to call this method
...
DAG node of this object with \texttt{Operation::MtoI}, and assign the
resulting edge the given \texttt{weight}. It is an error to call this method
...
intermediate DAG node of this object with \texttt{Operation::ItoI}, and
...
DAG node of this object with \texttt{Operation::ItoL} operation, and assign
...
has a default value of 0: \texttt{SingleLocality::SingleLocality(int loc = 0)}.
...
its operation on the sorts of DAG nodes or the operations perfomed on the DAG
...
that contains the view indicies, this will populate the size and data members
