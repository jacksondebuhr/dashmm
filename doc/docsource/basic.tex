...
The Source records will be copied across the network, and so if the type is
non-trivial, the user of DASHMM must provide a subclass of \texttt{Serializer}
to handle the serialization and deserialization of Source objects. See
section \ref{sec:serializer} for details.
...
and points can then be identified after the computation.

The Target records will be copied across the network, and so if the type is
non-trivial, the user of DASHMM must provide a subclass of \texttt{Serializer}
to handle the serialization and deserialization of Target objects. See
section \ref{sec:serializer} for details.
...
\texttt{dcomplex\_t} as an alias to \texttt{std::complex<double>}.
...
value broadcast from rank 0 will be stored. The type \texttt{T} must be
trivially copyable.
...
\begin{itemize}
\item \texttt{const Array<Source> \&sources}: the \texttt{Array} containing
  the source data.
\item \texttt{const Array<Target> \&targets}: the \texttt{Array} containing
  the target data.
...
\item \texttt{const Method<Source, Target, Expansion<Source, Target>> *method}:
  an instance of the method to use for the evaluation. A few methods require
  parameters at construction, so this is passed in to provide those parameters
  to the evaluation.
...
\item \texttt{const std::vector<double> *kernelparams}: the parameters for the
  ...
  of the particular expansion. See the individual expansions for details about
...
type over the record. For basic use of \texttt{Array} the record type should
be trivially copyable. For more advanced record types, see
section~\ref{sec:serializer}. When an \texttt{Array} is created, as in
...
This will return \texttt{kDomainError} if the object already has an allocation,
\texttt{kAllocationError} if memory is unable to be allocated,
\texttt{kRuntimeError} if there is an error with the runtime system, or
\texttt{kSuccess} if the method completes without a problem.
...
Because some DASHMM routines will modify the segments of arrays, the length
...
parameter. It is possible that this will return \texttt{nullptr}, but this will
...
of the \texttt{ArrayForEachAction} type. This type specifies an action to be
...
\texttt{ArrayForEachAction}. This is provided as a function pointer, for a
function with a particular signature. So that DASHMM can use the action on
every locality, like the \texttt{Evaluator} object, any
\texttt{ArrayForEachAction} objects must be defined before \texttt{init()}
is called.
...
\texttt{ArrayForEachAction}. For example:
...
the associated \texttt{ArrayForEachAction} must be defined before
\texttt{init()} is called. This object will register the needed actions with
the runtime system.

For any given action \texttt{f} only a single \texttt{ArrayForEachAction}
should be defined.
...
\noindent Once an \texttt{ArrayForEachAction} is defined, it can be used on a
specific array by calling that array's \texttt{map()} method. This will cause
the action represented by \texttt{act} to be applied on all entries of this
array. The action ultimately works on segments of the array. The environment,
\texttt{env} is provided unmodified to each segment.

This is a collective call. It is an error to call \texttt{forEach()} on an
invalid array.
...
computing the exact result for a given set of sources. There is some
parallelism implemented for this method, so the execution time is reduced
somewhat. However, for realistic problem sizes, this method should be avoided.
...
The \texttt{FMM} method implements the Fast Multipole Method in its original
form. To create an \texttt{FMM} method, no arguments are needed as the
decisions about which expansions to use in which situations are all made based
on fixed geometric considerations.
...
The \texttt{Helmholtz} expansion expands the Helmholtz potential in the
low-frequency regime using spherical harmonics, spherical Bessel and Hankel
functions. This potential is scaling variant, so a single kernel parameter
must be provided to \texttt{evaluate()}. Calls to evaluate must also supply an
accuracy parameter that gives the number of digits of accuracy required.

Though this expansion is designed for use with the \texttt{FMM97} method.
...
expansion can be used to compute the potential. See \texttt{LaplaceCOMAcc} for
an equivalent expansion that computes the acceleration. This potential is
...
This expansion imposes the following restrictions on the target type: a
member of type \texttt{Point} with the name \texttt{position} must be provided;
a member \texttt{double acceleration[3]} must be provided.
