...
std::complex<double> BH_Expansion::term(size_t i) const {
  if (i == 0) {
...
std::unique_ptr<Expansion> BH_Expansion::S_to_M(Point center,
                       std::vector<Source>::iterator first,
                       std::vector<Source>::iterator last) const {
  BH_Expansion *retval{new BH_Expansion{center}};
  assert(retval);
...
std::unique_ptr<Expansion> BH_Expansion::M_to_M(int from_child,
                                                double s_size) const {
  BH_Expansion *temp = new BH_Expansion(Point{0.0, 0.0, 0.0});
  temp->set_mtot(mtot_);
  temp->set_xcom(xcom_);
  temp->set_Q(Q_);
...
void BH_Expansion::M_to_T(std::vector<Target>::iterator first,
...
void BH_Expansion::S_to_T(std::vector<Source>::iterator s_first,
...
void BH_Expansion::add_expansion(const Expansion *temp1) {
  //This version does nothing for BH_Expansion
}


void BH_Expansion::from_sum(const std::vector<const Expansion *> &exps) {
...
void BH_Expansion::calc_mtot(std::vector<Source>::iterator first,
...
void BH_Expansion::calc_xcom(std::vector<Source>::iterator first,
                             std::vector<Source>::iterator last) {
  xcom_[0] = 0.0;
  xcom_[1] = 0.0;
  xcom_[2] = 0.0;
...
void BH_Expansion::calc_Q(std::vector<Source>::iterator first,
                          std::vector<Source>::iterator last) {
  for (int i = 0; i < 6; ++i) {
    Q_[i] = 0;
  }
